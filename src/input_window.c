...
int main(int argc, char *argv[]) {
    // [FIX 1] Force create the log file immediately so Watchdog sees it
    FILE *fp = fopen("log/input_window_log.txt", "w");
    if (fp) { fprintf(fp, "Process Started\n"); fclose(fp); }
    ...
    noecho(); 
    keypad(stdscr, TRUE); 
    timeout(100); // Non-blocking read (100ms delay)
    ...
    while(1) {
        ...
        // [FIX 2] Ensure this runs every loop cycle
