...
int main(int argc, char *argv[]) {
    log_process_pid("Watchdog"); 
    
    // --- FIX [Pipes]: Accept pipe FDs (even if monitoring is passive, pipes are now received) ---
    if (argc > 1) {
        // Just logging them to prove reception as per requirement
        log_debug_system("Watchdog received monitoring pipes: %s %s", argv[1], argv[2]);
    }
