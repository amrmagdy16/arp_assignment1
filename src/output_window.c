...
int main(int argc, char *argv[]) {
    ...
    while(1) {
        ...
        while(read(fd_rx, &tmp, sizeof(Game)) > 0) {
            ...
            char c = tmp.command;
            if(c) {
                // --- NEW: Handle Timer Reset 'o' ---
                if (c == 'o') {
                    start_time = time(NULL);
                    d.actual_score = 0;
                    log_debug_system("Manual Reset (o). Timer restarted.");
                }
        ...
        for(int i=0; i<MAX_TARGETS; i++) {
            if(g.Target_x[i] > 0 && abs((int)d.x - g.Target_x[i]) < 3 && abs((int)d.y - g.Target_y[i]) < 3) {
        ...
        log_execution("log/output_window_log.txt");
