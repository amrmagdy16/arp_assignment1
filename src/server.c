...
int main(int argc, char *argv[]) {
    log_process_pid("Server");

    int fd_in_rx  = atoi(argv[1]); // From Input
    int fd_obs_rx = atoi(argv[2]); // From Obstacle (or Network)
    int fd_tar_rx = atoi(argv[3]); // From Target
    int fd_out_rx = atoi(argv[4]); // From Output (Physics)
    int fd_in_tx  = atoi(argv[5]); // To Input (UI)
    int fd_out_tx = atoi(argv[6]); // To Output (Physics)
    // [EDIT]: Add optional Network Pipe
    int fd_net_tx = (argc > 7) ? atoi(argv[7]) : -1; 
    ...
    while (1) {
        ...
        // 1. Handle Obstacles (Local or Network)
        ...
        if (FD_ISSET(fd_out_rx, &read_fds)) {
            if(read(fd_out_rx, &drone, sizeof(Drone)) > 0) {
                ...
                // [EDIT]: Forward to Network (if active)
                if (fd_net_tx != -1) {
                    write(fd_net_tx, &drone, sizeof(Drone));
                }
            }
        }
