...
void calc_raw_repulsion(float dist, float dir_x, float dir_y, float *Px, float *Py) {
    if (dist < RHO) {
        ...
        if (magnitude > 200.0) magnitude = 200.0; 
        *Px += magnitude * dir_x;
        *Py += magnitude * dir_y;
    }
}
...
int main(int argc, char *argv[]) {
    ...
    Drone d = {COLS/2.0f, LINES/2.0f, 0, 0, 0, 0};
    Game g = {0};
    Game tmp = {0};
    ...
    while(1) {
        ...
        if (ch == KEY_RESIZE) {
            handle_resize();
            if(d.x >= COLS) d.x = COLS - 2;
            if(d.y >= LINES) d.y = LINES - 2;
        }
        ...
        while(read(fd_rx, &tmp, sizeof(Game)) > 0) {
            ...
            if(c) {
                ...
                if(d.cmd_fx > 15) d.cmd_fx = 15; if(d.cmd_fx < -15) d.cmd_fx = -15;
                if(d.cmd_fy > 15) d.cmd_fy = 15; if(d.cmd_fy < -15) d.cmd_fy = -15;
            }
        }
        ...
        if (Px != 0 || Py != 0) {
            ...
            if (best_k != -1 && max_dot > 0) {
                vfx = max_dot * UX[best_k];
                vfy = max_dot * UY[best_k];
            }
        }
        ...
        if(d.x < 1) d.x = 1.0; 
        if(d.x > COLS-2) d.x = COLS-2.0;
        if(d.y < 1) d.y = 1.0; 
        if(d.y > LINES-2) d.y = LINES-2.0;
